...
function Chat({ socket, username, room, setShowChat }) {
  ...
  useEffect(() => {
    const handleReceiveMessage = (data) => {
      ...
      // Sound notification if tab is hidden
      if (
  ...
  return (
    <div className="flex flex-col h-screen w-screen md:h-150 md:w-175 md:max-w-[95vw] bg-white md:rounded-2xl shadow-2xl overflow-hidden font-sans border border-gray-200">
      ...
      <div className="bg-gray-900 p-4 text-white flex justify-between items-center z-30 shadow-md">
        <div className="flex items-center gap-2">
          <button
            ...
            className="bg-red-500/20 hover:bg-red-600 text-red-500 hover:text-white px-3 py-1 rounded-lg text-[10px] font-bold transition-all border border-red-500/30"
          >
            LEAVE
          </button>
          <div>
            <h2 className="text-sm font-black tracking-tighter text-blue-400 uppercase">
              ChatAlong
            </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="md:hidden p-2 bg-gray-800 rounded-lg text-blue-400 text-[10px] font-bold border border-blue-400/20"
          >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div
          className={`
          absolute md:relative z-20 w-64 md:w-44 h-full bg-gray-50 border-r border-gray-200 p-5 transition-transform duration-300
          ...
          <div className="flex flex-col gap-3 overflow-y-auto">
            {userList.map((user, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs font-bold truncate ${user === username ? "text-blue-600" : "text-gray-700"}`}
                >
                  {user}
                </span>
              </div>
        ...
        <div className="flex-1 flex flex-col bg-white w-full relative">
                    ...
                    <div key={index}>
                      ...
                      {isSystem ? (
                        <div className="flex justify-center my-2">
                          <span className="text-[9px] text-gray-400 uppercase font-bold tracking-tighter italic">
                            {msg.message}
                          </span>
                        </div>
                      ) : (
                        ...
                        >
                          <div
                            className={`max-w-[85%] md:max-w-[75%] p-3 rounded-2xl shadow-sm ${isMe ? "bg-blue-600 text-white rounded-tr-none" : "bg-gray-100 text-gray-800 rounded-tl-none border border-gray-200"}`}
                          >
                            {!isMe && (
                            ...
                            <p
                              className={`text-[8px] mt-1 text-right font-bold ${isMe ? "text-blue-100" : "text-gray-400"}`}
                            >
                              {msg.time}
                            </p>
                          </div>
          ...
          <div className="p-3 bg-white border-t border-gray-100 flex items-center gap-2">
            <button
              ...
              className={`text-xl transition-all ${showEmojiPicker ? "scale-125" : "grayscale hover:grayscale-0"}`}
            >
              😊
            </button>
            ...
            <button
              onClick={sendMessage}
              className="bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-700 font-black text-xs tracking-widest transition-all active:scale-95 shadow-lg shadow-blue-500/20"
            >
              SEND
            </button>
          </div>
